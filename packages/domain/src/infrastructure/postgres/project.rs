...
#[derive(Debug)]
pub struct PostgresProject<C: Contract> {
    pub db_client_pool: Arc<Pool>,
    contract: std::marker::PhantomData<C>,
}

impl<C> PostgresProject<C>
where
...
impl PostgresProject<Erc3525> {
    ...
    pub async fn create(
        ...
        let value_decimals = match data.get_mut("value_decimals") {
            None => U256(crypto_bigint::U256::from_u64(0)).into(),
            Some(v) => v.resolve("u256").into(),
        };
        let project_value = match data.get_mut("get_project_value") {
            None => U256(crypto_bigint::U256::from_u64(0)).into(),
            Some(v) => v.resolve("u256").into(),
        };
        let (sql, values) = Query::insert()
            ...
            .values([
                ...
                data.get_mut("owner")
                    ...
                    .resolve("address")
                    .into(),
                data.get_mut("get_ton_equivalent")
                    .expect("ton equivalent has to be provided")
                    .resolve("u256")
                    .into(),
                data.get_mut("get_times")
                    .expect("getTimes has to be provided")
                    .resolve("datetime_array")
                    .into(),
                data.get_mut("get_absorptions")
                    .expect("getAbsorptions has to be provided")
                    .resolve("u256_array")
                    .into(),
                data.get_mut("is_setup")
                    .expect("isSetup has to be provided")
                    .resolve("bool")
                    .into(),
                value_decimals,
